using CatalogoProductos.Client.Shared;
...
namespace CatalogoProductos.Client.Services
{
    ...
    public class Api
    {
        ...
        public async Task<RespuestaClassDto<TOut>>? GetApiAuth<TOut>(string ruta, bool verRespuestaCorrecta = false, int timeOut = 2)
        {
            try
            {
                ...
                using (var timeoutCancellationTokenSource = new CancellationTokenSource(TimeSpan.FromMinutes(timeOut)))
                {
                    ...
                    {
                        Console.WriteLine($"Error al obtener los datos del API: {httpResponse.StatusCode}");
                        Modal!.OpenModalError(_mensajeError, _detalleError);
                        return default!;
                    }

                    var respuesta = JsonConvert.DeserializeObject<RespuestaClassDto<TOut>>(content);
                    if (respuesta == null)
                    {
                        Console.WriteLine("La respuesta deserializada es nula.");
                        return default!;
                    }
        ...
        private async Task HttpHeadersAuth()
        {
            ...
            _httpClient.DefaultRequestHeaders.Add("Sitio", "CatalogoProductos");
